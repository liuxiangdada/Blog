...
7.状态图：每一种状态表示一种可能的状态赋值

8.有限状态机：包含有限数量的状态，模型可以表示动作序列

9.状态空间：对象每个属性的取值空间的乘积加1
...
11.状态：一个对象生命期的一个阶段，该阶段中对象要满足一些特定的条件、执行特定的活动或等待某个事件的发生

12.事件：可以触发对象状态改变的外部刺激，也就是消息的发出与接收
...
14.UML图中的组合状态（超级状态）：文法上的转换，表达的含义相同，分为OR组合（只能满足一个）和AND组合（并发）

15.UML图中的历史状态：当激活这个状态时，会保存从组合状态退出时所处的子状态，再次进入组合状态时，可直接进入到这个子状态
