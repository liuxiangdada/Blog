...
举个例子，Array新增的from方法，就无法被babel转译，我们只能在使用该方法的文件开头加上
...
1. 淘汰了es201x，移除了stage-X，强推env，这意味着env容纳了前面所有的插件列表以及以后落地的插件
2. 对npm包进行重命名，增加了命名空间`@babel`用以避免被其他库影响污染，类似于jquery的做法
...
babel-upgrade升级工具主要是检查package.json文件和.babelrc文件

- 对其中的babel版本升级到最新版
- 将`babel-*`替换为`@babel/*`
- 如果使用了babel-node，自动添加@babel/node依赖
- 检查babel的插件preset配置，将env替换为完整的@babel/preset-env
...
前面我们说过，babel的编译过程可以拆解为 解析 -> 转换 -> 生成 3个部分，接下来我们一一分析一下这3个过程
...
transform：接收字符串形式的代码作为第一个参数，后面是可选的配置项，也就是平时我们在.babelrc中的配置，输出转化后的字符串代码、sourcemap和AST
    ...
    console.log(res.code, res.AST, res.sourcemap)

### babel-types

这个模块是用来操作AST节点的Lodash式工具库，包含了处理节点的各种方法 [传送门](https://babeljs.io/docs/en/babel-types#docsNav)
...
[Babel原理](https://github.com/webfansplz/article/issues/2)
