## Vue2.x源码分析

[阅读源码前你应该了解哪些知识](./docs/vue2.x/prepare.md)

[Vue是如何实现数据绑定的](./docs/vue2.x/reactive.md)

[Vue是如何更新视图的](./docs/vue2.x/update.md)

[Vue是如何实现组件的](./docs/vue2.x/component.md)

[Vue是如何编译模板的](./docs/vue2.x/compile.md)

[Vue的一些扩展功能](./docs/vue2.x/extend.md)

## Vuex源码分析

[Vuex数据仓库是如何构建的](./docs/vuex/store.md)

[Vuex常用API](./docs/vuex/api.md)

## VueRouter源码分析

[vue-router插件的安装过程](./docs/vue-router/install.md)

[vue-router核心-路由匹配器](./docs/vue-router/matcher.md)

[vue-router的导航守卫](./docs/vue-router/guard.md)

[vue-router提供的全局组件](./docs/vue-router/global-component.md)

[vue-router的路由模式](./docs/vue-router/history.md)

