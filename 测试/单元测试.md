...
```diff
这里我在配置babel-jest遇到了很多坑，初步看可能是我项目中使用的是老的babel6，但是jest现在支持babel7的语法导致的兼容问题，
暂时没有解决，只能使用@vue/cli-plugin-unit-jest，使用这个插件不需要配置babel也不需要安装babel-jest和vue-jest，
因为插件内部已经使用了这两个包
```
